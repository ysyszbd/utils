 ...
 * @return {*}
 */
 export function dateFormat(time: any, format: string = 'yyyy-mm-dd h:m:s') {
  let timestamp = time.length === 10 ? time * 1000 : time,
      date = new Date(timestamp),
      undefinedArray: [string, number][] | undefined,
      dateObj = new Map(undefinedArray);
 ...
 * @return {*}
 */
 export function  random_string(len: number = 32) {
 ...
 */
 export function getUrlParamItem(name: string, url = location.href) {
  let reg = new RegExp('(^|&)' + name + '=([^&]*)(&|$)', 'i'),
      r = url.substring(url.indexOf('?')).substring(1).match(reg);
 ...
 * @return {*} true / false
 */
 export function isPhoneNum(mobile: string) {
